...
public class StoneHolderMenu extends AbstractContainerMenu {
    ...
    private static final int STONE_SLOT_X = 80;
    ...
    public StoneHolderMenu(int id, Inventory playerInventory, ItemStack holderStack) {
        ...
        for (int i = 0; i < 3; i++) this.addSlot(new DummySlot(i));
        this.addSlot(new StoneSlot(itemHandler, 0, STONE_SLOT_X, STONE_SLOT_Y, holderStack));
        for (int i = 4; i < 6; i++) this.addSlot(new DummySlot(i));
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            if (index == STONE_SLOT_INDEX) {
                ...
                if (this.slots.get(STONE_SLOT_INDEX).mayPlace(slotStack)
                        && this.slots.get(STONE_SLOT_INDEX).getItem().isEmpty()) {
    ...
    private static class DummySlot extends Slot {
        public DummySlot(int index) { super(DUMMY_CONTAINER, index, -1000, -1000); }
    ...
    public static class Provider implements MenuProvider {
        ...
        public Provider(ItemStack stack) { this.stack = stack; }
        @Override public Component getDisplayName() { return Component.literal("Stone Holder"); }
